...
def main():
    ...
    if copy_files == 'true':
        for frame in range(0, num_frame):
            ...
            subprocess.run(["scp", "root@" + dut_ip + ":/tmp/layers/" + str(frame) + "/*-kActIdentity_output.txt", "./" + str(frame) + "/identity/"])
            subprocess.run(["scp", "root@" + dut_ip + ":/tmp/ref_layers/" + str(frame) + "/*-kActIdentity_output.txt", "./" + str(frame) + "/identityfp32"])
    ...
    if parse_graph == 'true':
        ...
        for frame in range(0, num_frame):
            ...
            fp32_layers_op = []
            file_count = 0
            for filename in os.listdir(os.getcwd() + "/" + str(frame) + "/identityfp32/0"):
                layers_op.append([])
                fp32_layers_op.append([])
                fp32_layers_op[file_count].append(filename)
                layers_op[file_count].append(filename)

                with open(os.getcwd() + "/" + str(frame) + "/identity/0/" + str(filename), 'r') as f:
                    layer_op = []
                    output_list = []
                ...
                with open(os.getcwd() + "/" + str(frame) + "/identityfp32/0/" + str(filename), 'r') as f:
                    fp32_layer_op = []
                    fp32_output_list = []
            ...
            for i in range(0, len(layers_op)):
                ...
                over_flow_count += item[2].count("-63.998")
